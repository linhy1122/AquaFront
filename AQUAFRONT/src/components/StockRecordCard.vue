<template>
  <div class="card">
    <div class="card-header">
      <h3>放养记录</h3>
      <button class="btn btn-primary btn-sm" @click="openAddDialog">+ 新增放养</button>
    </div>
    <div class="card-body">
      <div class="toolbar">
        <div class="search-box">
          <input type="text" v-model="searchForm.pondName" placeholder="搜索塘口..." @keyup.enter="fetchRecords">
          <select v-model="searchForm.species" @change="fetchRecords">
            <option value="">全部品种</option>
            <option v-for="s in speciesList" :key="s" :value="s">{{ s }}</option>
          </select>
          <button class="btn btn-primary btn-sm" @click="fetchRecords">搜索</button>
        </div>
      </div>
        ...
        <table>
          ...
          <tbody>
            <tr v-if="records.length === 0">
              <td colspan="9" style="text-align:center;padding:40px;color:#999;">暂无放养记录</td>
            </tr>
...
import { stockingApi } from '../api'

defineOptions({ name: 'StockRecordCard' })
...
const fetchRecords = async () => {
  try {
    const res = await stockingApi.list({
      page: page.value,
      size: size.value,
      pondName: searchForm.pondName || undefined,
